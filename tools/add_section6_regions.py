    ...
    cells.append(
        code(
            [
                ...
                "else:",
                "    # Infer region from instrument metadata using simple heuristics.",
                "    def infer_region(label: str) -> str:",
                "        s = label.lower()",
                "        # Priority order: US, Europe, UK, Japan, China, Canada, Australia, EM, Global/Other",
                "        if any(k in s for k in [' us ', ' us equity', 'usd', 'united states', 'treasury', ' usgg', ' spx ', ' spy ']):",
                "            return 'US'",
                "        if any(k in s for k in [' euro', ' eur', 'europe', 'stoxx', 'sx5e', 'dax ']):",
                "            return 'Europe'",
                "        if any(k in s for k in [' uk ', 'ftse', ' gbp', ' united kingdom']):",
                "            return 'UK'",
                "        if any(k in s for k in [' japan', ' jpy', ' nky ', 'tpx ']):",
                "            return 'Japan'",
                "        if any(k in s for k in [' china', ' cny', ' cnh', ' shcomp', ' hscei']):",
                "            return 'China'",
                "        if any(k in s for k in [' canada', ' cad', ' tsx']):",
                "            return 'Canada'",
                "        if any(k in s for k in [' australia', ' aud', ' asx']):",
                "            return 'Australia'",
                "        if any(k in s for k in [' emerging', ' msci em', ' eem ']):",
                "            return 'EM'",
                "        if any(k in s for k in [' world', ' global', ' acwi', ' vt ']):",
                "            return 'Global'",
                "        return 'Global'",
                "",
